...
Original file is located at
    https://colab.research.google.com/drive/1wZwoUXImemkV8nyJNs_OokcRoi-Bn7H5
...
!gdown --id '1-Kmb4qZTFvE1JaA_JAFUuQx6cv2rpXOa' --output data-bin.tar.gz
...
class conv_autoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(3, 80, 4, stride=2, padding=1),      
            nn.ReLU(),
            nn.Conv2d(80, 200, 4, stride=2, padding=1),   
            nn.ReLU(),
			      nn.Conv2d(200, 420, 4, stride=2, padding=1),      
            nn.ReLU(),
        )
        self.decoder = nn.Sequential(
			nn.ConvTranspose2d(420, 200, 4, stride=2, padding=1),
            nn.ReLU(),
			nn.ConvTranspose2d(200, 80, 4, stride=2, padding=1), 
            nn.ReLU(),
            nn.ConvTranspose2d(80, 3, 4, stride=2, padding=1),
            nn.Tanh(),
        )
...
num_epochs = 170
batch_size = 3000 # medium: smaller batchsize
learning_rate = 1e-3
